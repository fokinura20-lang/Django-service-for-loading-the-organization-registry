...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        file_load = FileLoad.objects.create(filename=filepath, status="PROCESSING")
                ...
                try:
                    with transaction.atomic():
                        Organization.objects.create(
                            inn=inn, name=name, region=region, employees=employees, registration_date=reg_date
                        )
                        loaded += 1
                    ...
                    duplicates += 1
                    LoadError.objects.create(
                        load=file_load, row_number=row_num, raw_data=raw_data, error_message=f"Дубликат ИНН: {inn}"
                    )
